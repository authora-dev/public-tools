        ...
        <div className="space-y-2">
          {questions.map((q, i) => {
            ...
            return (
              ...
              >
                <span className="text-[var(--color-dim)] text-xs w-5 text-right flex-shrink-0">{i + 1}.</span>
                <span className={`flex-1 text-sm leading-snug ${checked ? 'text-emerald-400' : 'text-[#ccc]'}`}>
                  {q.text}
                </span>
                <div className="flex gap-1 flex-shrink-0">
                  <button
                    type="button"
                    onClick={() => { if (!checked) toggleAnswer(q.id) }}
                    className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${
                      checked
                        ? 'bg-emerald-500 text-white'
                        : 'bg-[var(--color-card)] text-[var(--color-dim)] hover:bg-emerald-500/20 hover:text-emerald-400 border border-[var(--color-border)]'
                    }`}
                  >Yes</button>
                  <button
                    type="button"
                    ...
                    className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${
                      !checked
                        ...
                        : 'bg-[var(--color-card)] text-[var(--color-dim)] hover:bg-red-500/20 hover:text-red-400 border border-[var(--color-border)]'
                    }`}
                  >No</button>
                </div>
              </div>
